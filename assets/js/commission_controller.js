...
// commissionApp.filter('checkFilter', function() {
...
commissionApp.controller('commission',['$scope','socket','$timeout','$sce','usSpinnerService','$interval','$filter','$rootScope', function ($scope,socket,$timeout,$sce,usSpinnerService,$interval,$filter,$rootScope) {
  ...
  // $scope.$watch($('#editor').cleanHtml(), function(newValue, oldValue) {
  //   if (newValue!=null && oldValue!=null)
  //     showmsg = true;
  // });
  ...
  (function () {
    ...
    socket.get('/commission/templateinfo/'+mastereventid)
      .then(function(resp){
         ...
         $scope.eventinfo = resp.data.eventinfo;
    ...
    socket.get('/commission/allshots/')
      .then(function(resp){
